...
Then /^I should see the class as "(.*?)"$/ do |status|
  find('table#inscriptions-table > tbody > tr:first-child > td:nth-child(5)').should have_content(status)
end
...
Then /^I should not be able to enroll$/ do
  find('table#inscriptions-table > tbody > tr:first-child > td:nth-child(7)').should_not have_css('input[value="Anotarse"]')
end

Then /^I should be able to enroll$/ do
  find('table#inscriptions-table > tbody > tr:first-child > td:nth-child(7)').should have_css('input[value="Anotarse"]')
end
...
Then /^I enroll into the class$/ do
  ...
  find('table#inscriptions-table > tbody > tr:first-child > td:nth-child(7)').should have_css('input[value="Liberar"]')
  find('table#inscriptions-table > tbody > tr:first-child > td:nth-child(4)').should have_content('1')
end
